    ...
    <div className="w-full">
        <div className="pb-4 sm:px-6 flex flex-col flex-wrap justify-center content-center bg-[#DEDEDE] font-poppins">
            <div className="flex justify-center sm:mb-3 mb-5">
                <h1 className="sm:text-3xl text-xl font-meduim text-center sm:mx-20 my-7 w-5/6"> {article.title}</h1>
            </div>
            ...
            <div className="flex justify-center flex-col sm:px-4 px-6 sm:my-5 py-1">
                <div className="flex content-center flex-nowrap my-3">
                    ...
                    <p className='text-[#190B28] text-base font-semibold mr-1'>Auteurs:</p>
                ...
                <div className="flex content-center flex-nowrap">
                    <LiaPenFancySolid  className="h-6 w-6 mr-1"/>
                    <p className='text-[#190B28] text-base font-semibold mr-1'>Institutions:</p>
                    <p className='text-[#190B28] text-base '>
                        {article.authors.map(auteur =>{
            ...
            <div className="flex justify-center my-1 flex-col sm:px-4 px-6 py-2">
                <p className="text-2xl sm:text-3xl font-meduim sm:my-2 mb-3 mx-1">Abstract:</p>
                <p className='text-[#190B28] text-base sm:text-sm text-justify mt-2 mb-5 w-full'>{article.abstract} </p>
            </div>

            <hr className="bg-[#E87D00] h-1 sm:h-0.5 w-full rounded-full border-none sm:mb-0 mb-2"/>

            <div className="flex justify-center mb-3 flex-col sm:px-4 px-6 py-2 mt-3">
                <p className="text-2xl sm:text-3xl font-meduim sm:my-2 mx-1">Article:</p>
                <div className="sm:grid grid-cols-2 gap-10 px-2 mb-5 sm:mt-6 mt-4 hidden">
                    <div><p className='text-[#190B28] text-base sm:text-sm text-justify' >{displayWordRange(texte,0,len6+1)}</p></div>
                    <div><p className='text-[#190B28] text-base sm:text-sm col-span-1 text-justify' > {displayWordRange(texte,(len6)+1,(2*len6)+1)}</p></div>
                </div>
                <div className="sm:grid grid-cols-2 gap-10 px-2 mb-5 hidden">
                    <div><p className='text-[#190B28] text-base sm:text-sm text-justify' >{displayWordRange(texte,(2*len6)+1,(3*len6)+1)}</p></div>
                    <div><p className='text-[#190B28] text-base sm:text-sm col-span-1 text-justify' > {displayWordRange(texte,(3*len6)+1,(4*len6)+1)}</p></div>
                </div>
                <div className="sm:grid grid-cols-2 gap-10 px-2 mb-7 hidden">
                    <div><p className='text-[#190B28] text-base sm:text-sm text-justify' >{displayWordRange(texte,(4*len6)+1,(5*len6)+1)}</p></div>
                    <div><p className='text-[#190B28] text-base sm:text-sm col-span-1 text-justify' > {displayWordRange(texte,(5*len6)+1,len+1)}</p></div>
                </div>

                {/* phone view */}
                <div className="px-2 mb-2 sm:hidden sm:mt-6 mt-4">
                    <div><p className='text-[#190B28] text-base sm:text-sm text-justify' >{texte}</p></div>
                </div>
